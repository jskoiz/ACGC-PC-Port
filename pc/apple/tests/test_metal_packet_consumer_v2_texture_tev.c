...
int main(void) {
    ...
    CHECK(acgc_metal_packet_consumer_prepare_v2_texture_tev(
              &packet, &fixture, 1, &output) ==
          ACGC_METAL_PACKET_CONSUMER_OK);
    CHECK(output.v2_extension_rendering_status ==
    ...
    /* Swap table 1 exchanges red and green at the TEV texture input. */
    CHECK(output.v2_tev_color.r == 0 && output.v2_tev_color.g == 255 &&
          output.v2_tev_color.b == 0 && output.v2_tev_color.a == 255);
    CHECK(output.geometry.vertices[0].color_rgba8 == 0x00FF00FF);
    CHECK(output.geometry.vertices[1].color_rgba8 == 0x00FF00FF);
    CHECK(output.geometry.vertices[2].color_rgba8 == 0x00FF00FF);

    /* Missing bytes and unsupported base-level mip sampling fail closed. */
    fixture.data = NULL;
    status = acgc_metal_packet_consumer_prepare_v2_texture_tev(
        &packet, &fixture, 1, &output);
    CHECK(status == ACGC_METAL_PACKET_CONSUMER_TEXTURE_FIXTURE_INVALID);
    set_texture_fixture(&fixture, texture_data, tlut_data, decoded_rgba);
    fixture.sampler.min_filter = ACGC_RENDERER_FIXTURE_FILTER_NEAR_MIP_NEAR;
    status = acgc_metal_packet_consumer_prepare_v2_texture_tev(
        &packet, &fixture, 1, &output);
    CHECK(status == ACGC_METAL_PACKET_CONSUMER_TEXTURE_FIXTURE_INVALID);

    /* Packet validation remains the first gate for unsupported TEV inputs. */
    set_texture_fixture(&fixture, texture_data, tlut_data, decoded_rgba);
    packet.tev_stages[0].color_input[0] = 99;
    status = acgc_metal_packet_consumer_prepare_v2_texture_tev(
        &packet, &fixture, 1, &output);
    CHECK(status == ACGC_METAL_PACKET_CONSUMER_INVALID_PACKET);

    puts("Apple v2 texture/TLUT/TEV CPU consumer fixture: PASS");
    puts("proof boundary: synthetic CPU decode/evaluation only; no Metal encode, frame, pixel, or playability claim");
    return 0;
}
