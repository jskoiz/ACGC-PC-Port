...
static const char ACGC_METAL_PACKET_CONSUMER_SHADER[] =
    ...
    "    AcgcRendererVertex vertex = vertices[vertex_id];\n"
    ...
    "    Output output;\n"
    "    output.position = matrix * float4(as_type<float>(vertex.position_x), as_type<float>(vertex.position_y), as_type<float>(vertex.position_z), 1.0f);\n"
    "    output.color = float4(float((vertex.color_rgba8 >> 24) & 0xffu), float((vertex.color_rgba8 >> 16) & 0xffu), float((vertex.color_rgba8 >> 8) & 0xffu), float(vertex.color_rgba8 & 0xffu)) / 255.0f;\n"
    "    return output;\n"
    "}\n"
