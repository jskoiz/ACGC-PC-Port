...
static f32 aBridge_player_check(STRUCTURE_ACTOR* bridge, GAME_PLAY* play) {
    ...
    if ((dist0 > -mFI_UNIT_BASE_SIZE_F && dist0 < mFI_UNIT_BASE_SIZE_F) &&
        ...
        if (bridge_run_speed > bridge->arg0_f) {
            ...
            return add_calc(&bridge->arg1_f, 1.0f, 0.1f + (f32)GETREG(TAKREG, 24) * 0.01f, 0.1f, 0.01f);
        }
    }

    return add_calc(&bridge->arg1_f, 0.0f, (0.08f + (f32)GETREG(TAKREG, 21) * 0.01f) * (0.4f + bridge->arg0_f * 0.1f), 0.1f,
                   0.01f);
}
