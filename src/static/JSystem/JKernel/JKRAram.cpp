...
bool JKRAram::checkOkAddress(u8* addr, u32 size, JKRAramBlock* block, u32 blockSize) {
    if (!IS_ALIGNED(JKRNativeAddressOf(addr), 0x20) && !IS_ALIGNED(size, 0x20)) {
...
JKRAramBlock* JKRAram::mainRamToAram(u8* buf, u32 address, u32 alignedSize, JKRExpandSwitch expandSwitch, u32 fileSize,
    ...
    if (expandSwitch == EXPAND_SWITCH_DECOMPRESS) {
        ...
        if (allocatedMem == nullptr) {
            ...
            JKRAramPcsNative(0, JKRNativeAddressOf(allocatedMem), address, alignedSize, block);
        ...
        JKRAramPcsNative(0, JKRNativeAddressOf(buf), address, alignedSize, block);
        block = block == nullptr ? (JKRAramBlock*)-1 : block;
    }
    return block;
}
...
u8* JKRAram::aramToMainRam(u32 address, u8* buf, u32 srcSize, JKRExpandSwitch expandSwitch, u32 p5, JKRHeap* heap,
    ...
    if (expandSwitch == EXPAND_SWITCH_DECOMPRESS) {
        ...
        JKRAramPcsNative(1, JKRNativeAddressOf(bufPtr), address, sizeof(buffer) / 2,
    ...
    } else if (compression == JKRCOMPRESSION_YAY0) // SZP
    {
        ...
        if (szpSpace == nullptr) {
            ...
            JKRAramPcsNative(1, JKRNativeAddressOf(szpSpace), address, srcSize, nullptr);
            if (p5 != 0 && p5 < expandSize)
                expandSize = p5;
    ...
    } else // Not compressed or ASR
    {
        ...
        if (buf == nullptr) {
            ...
            changeGroupIdIfNeed(buf, id);
            JKRAramPcsNative(1, JKRNativeAddressOf(buf), address, srcSize, nullptr);
            if (pSize != nullptr) {
                *pSize = srcSize;
            }
            return buf;
...
static u8* firstSrcData() {
    ...
    JKRAramPcsNative(1, JKRNativeAddressOf(buf), srcAddress + srcOffset, ALIGN_NEXT(transSize, 32), nullptr);
...
u8* nextSrcData(u8* current) {
    ...
    JKRAramPcsNative(1, JKRNativeAddressOf(dest + left), srcAddress + srcOffset,
