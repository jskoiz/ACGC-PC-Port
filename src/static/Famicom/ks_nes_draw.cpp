...
void ksNesDrawMakeBGIndTex(ksNesCommonWorkObj* wp, u32 mmc3) {
    ...
    for (row = 8; row < 236; row++) {
        ...
        for (col = 0; col < 34; col++) {
            ...
            if (ksNesNametableIsPacked(nametable_p)) {
                const u32 packedNametable = ksNesNametablePackedValue(nametable_p);
                nibble_acc = (packedNametable & 3) | (nibble_acc << 4);
                tile_byte = (packedNametable >> 8) & 0xFF;
            } else {
